                ...
                <div class="relative grid grid-cols-1 gap-8 p-8 lg:grid-cols-2 lg:p-12">
                    <div class="flex flex-col justify-center">
                        ...
                        <h1 class="mt-5 text-4xl font-semibold leading-tight text-white sm:text-5xl">
                            Suscripción activa: {{ auth()->user()->client->plans()->first()->nombre }}
                        </h1>

                        <div class="flex items-center gap-5">

                            <p class=" flex-item mt-4 max-w-xl text-sm text-white font-semibold ">
                                Tu suscripción: {{ auth()->user()->client->plans()->first()->precio }} €/mes - {{
                                auth()->user()->client->plans()->first()->pivot->estado }}

                            </p>
                            <p class=" flex-item mt-4 max-w-xl text-sm text-white font-semibold ">
                                Fecha de caducidad: {{ auth()->user()->client->plans()->first()->pivot->fecha_fin }}
