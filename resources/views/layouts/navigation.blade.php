<nav x-data="{ open: false }" class="sticky top-0 z-40 border-b border-white/10 bg-black/95 backdrop-blur">
    <div class="mx-auto flex h-16 w-full max-w-[1600px] items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <div class="hidden items-center gap-3 md:flex">
            <a href="{{ route('profile.edit') }}" class="rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-sm text-zinc-300 hover:border-violet-400/40 hover:text-white">
                Perfil
            </a>
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="rounded-lg bg-violet-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-violet-500">
                    Cerrar sesión
                </button>
            </form>
        </div>
    ...
    <div x-cloak :class="open ? 'block' : 'hidden'" class="border-t border-white/10 bg-black/95 md:hidden">
        <div class="space-y-1 px-4 py-3">
            ...
            @elseif (auth()->user()->role === 'client')
                <a href="{{ route('clients.dashboard') }}" class="block rounded-lg px-3 py-2 text-sm text-zinc-200 hover:bg-white/10">Mi panel</a>
            @elseif (auth()->user()->role === 'entrenador')
                <a href="{{ route('entrenadors.dashboard') }}" class="block rounded-lg px-3 py-2 text-sm text-zinc-200 hover:bg-white/10">Panel entrenador</a>
            @endif

            <a href="{{ route('profile.edit') }}" class="block rounded-lg px-3 py-2 text-sm text-zinc-200 hover:bg-white/10">Perfil</a>
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="mt-1 w-full rounded-lg bg-violet-600 px-3 py-2 text-left text-sm font-medium text-white hover:bg-violet-500">
                    Cerrar sesión
                </button>
            </form>
        </div>
